...
import br.com.edsontofolo.libraryapi.api.dto.ReturnedLoanDTO;
...
@ActiveProfiles("test")
@WebMvcTest(controllers = {LoanController.class}) // controller define quais controllers serão criados para o test, no caso somente o LoanControllerTest.class
...
public class LoanControllerTest {
    ...
    public void createLoanTest() throws Exception {
        LoanDTO dto = LoanDTO.builder().isbn("123").customer("Edson").email("edson@email.com").build();
        String json = new ObjectMapper().writeValueAsString(dto);

        Book book = Book.builder().id(1L).isbn("123").build();
        BDDMockito
                .given(bookService.getByIsbn("123"))
                .willReturn(Optional.of(book));

        Loan loan = Loan.builder().id(1L).customer("Edson").book(book).loanDate(LocalDate.now()).build();
        BDDMockito
    ...
    public void invalidIsbnCreateLoanTest() throws Exception {
        LoanDTO dto = LoanDTO.builder().isbn("123").customer("Edson").build();
        String json = new ObjectMapper().writeValueAsString(dto);
        ...
        mvc.perform(request)
                ...
                .andExpect(jsonPath("errors[0]").value("Book not found for entered isbn"));
    }

    @Test
    @DisplayName("Must throws an exception to try make loan of loaned book")
    ...
    @Test
    @DisplayName("Must return book")
    public void returnBookTest() throws Exception {
        // cenario (returned = true)
        ReturnedLoanDTO dto = ReturnedLoanDTO.builder().returned(Boolean.TRUE).build();
        String json = new ObjectMapper().writeValueAsString(dto);

        Loan loan = Loan.builder().id(1L).build();
        BDDMockito
                .given(loanService.findById(Mockito.anyLong()))
    ...
    @Test
    @DisplayName("Must return 404 when try return a invalid book")
